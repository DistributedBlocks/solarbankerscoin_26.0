...
function startSkycoin() {
  if (!dev) {
    ...
    skycoin.on('close', (code) => {
      // log.info('solarbankerscoin closed');
    ...
    skycoin.on('exit', (code) => {
      // log.info('solarbankerscoin exited');
    ...
    app.emit('solarbankerscoin-ready', { url: currentURL });
    ...
    submenu: [
      {
        ...
        click: () => {
          if (walletsFolder) {
            ...
            shell.showItemInFolder(path.join(app.getPath("home"), '.solarbankerscoin', 'wallets'));
          }
        },
      },
      {
        ...
        click: () => {
          if (walletsFolder) {
            ...
            shell.showItemInFolder(path.join(app.getPath("home"), '.solarbankerscoin', 'logs'));
          }
        },
      },
      {
